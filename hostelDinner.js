...
const HostelDinner = ({ navigation }) => {
  const [foodItems, setFoodItems] = useState([]);
  const { addToCart, clearCart } = useContext(CartContext);
  const fetchFoodItems = async () => {
    try {
      const response = await axios.get(
        "http://159.89.203.249:2001/api/food-items/list?canteenId=643bf48c34379c74054a99f4&mealType=dinner"
      );
  ...
  useEffect(() => {
    const canteenId = "643bf48c34379c74054a99f4";
  ...
  const [page, setPage] = useState("hostelDinner");
  ...
  useEffect(() => {
    const backAction = () => {
      if (page !== "hostelDinner") {
        setPage("hostelDinner");
        clearCart();
        return true;
      }
      return false;
            ...
            <View style={styles.menuContainer}>
              <View style={styles.headerContainer}>
                <View style={styles.menuHeader}>
                  <Text style={styles.menuHeaderText}>DinnerFood Menu</Text>
                </View>
