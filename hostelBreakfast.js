...
const HostelBreakfast = ({ navigation }) => {
  const [page, setPage] = useState("hostelBreakfast");
  const [foodItems, setFoodItems] = useState([]);
  const { addToCart, clearCart } = useContext(CartContext);
  const fetchFoodItems = async () => {
    try {
      const response = await axios.get(
        "http://192.168.1.7:3000/api/food-items/list?canteenId=643bf48c34379c74054a99f4&mealType=breakfast"
      );
  ...
  useEffect(() => {
    const backAction = () => {
      if (page !== "hostelBreakfast") {
        setPage("hostelBreakfast");
        clearCart();
        return true;
      }
      return false;
...
export default HostelBreakfast;
